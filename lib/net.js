...
// import
try {
    var OS = require('os'), spawn = require('child_process').spawn;
            ...
            .on(
                    ...
                    function(data) {
                        ...
                        for (var i = 0, il = lines.length; i < il; i++) {
                            ...
                            if (cap) {
                                cache = [parseInt(cap[1]) + cache[0],
                                        parseInt(cap[2]) + cache[1],
                                        parseInt(cap[4]) + cache[2],
                                        parseInt(cap[5]) + cache[3]];
                            }
                        }
                        stat = cache;
                        timer += 1;
            ...
            .on(
                    ...
                    function(data) {
                        ...
                        for (var i = 0, il = lines.length; i < il; i++) {
                            ...
                            if (cap) {
                                if (story == 0) {
                                    story = [parseInt(cap[4]),parseInt(cap[5]),
                                            parseInt(cap[8]),parseInt(cap[9])];
                                }
                                cache = [
                                        parseInt(cap[4]) + cache[0] - story[0],
