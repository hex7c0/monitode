...
try {
    var HTTPS = require("https"), FS = require("fs"), EXPRESS = require("express"), status = require("http").STATUS_CODES;
...
module.exports = function() {
    ...
    var log, net, io, options = global.monitode, h = options.https, app = EXPRESS(), end = without_log;
    ...
    app.use(EXPRESS["static"](dir)), FS.existsSync(h.key) && FS.existsSync(h.cert) && (options.output && console.log("starting monitor on port " + h.port), 
    HTTPS.createServer({
        key: FS.readFileSync(h.key),
        cert: FS.readFileSync(h.cert)
    }, app).listen(h.port)), global.monitode.https = !0, app.get("/", function(req, res) {
